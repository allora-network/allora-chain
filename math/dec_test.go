	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.a == "NaN" {
				...
				a, err = alloraMath.NewDecFromString(tc.a)
			...
			if tc.b == "NaN" {
				...
				b, err = alloraMath.NewDecFromString(tc.b)
			...
			if tc.shouldError {
				...
				expected, err := alloraMath.NewDecFromString(tc.expectedStr)
				require.NoError(t, err, "Failed to parse expected result: %s", tc.expectedStr)

				// Check if result is within tolerance
				if tc.tolerance != "" {
					tolerance, err := alloraMath.NewDecFromString(tc.tolerance)
					require.NoError(t, err, "Failed to parse tolerance: %s", tc.tolerance)

					withinTolerance, err := alloraMath.InDelta(expected, result, tolerance)
...
func TestExp1DivExp1SpecialProperties(t *testing.T) {
	t.Run("Identity: f(a,a) = 1", func(t *testing.T) {
		...
		for _, val := range testValues {
			a, err := alloraMath.NewDecFromString(val)
			require.NoError(t, err)

			result, err := alloraMath.Exp1DivExp1(a, a)
			require.NoError(t, err)

			one := alloraMath.OneDec()
			tolerance, _ := alloraMath.NewDecFromString("0.0000000001")
			withinTolerance, err := alloraMath.InDelta(one, result, tolerance)
			require.NoError(t, err)
			require.True(t, withinTolerance,
	...
	t.Run("Monotonicity: If a1 < a2 and b fixed, then f(a1,b) < f(a2,b)", func(t *testing.T) {
		b, _ := alloraMath.NewDecFromString("1")
		a1, _ := alloraMath.NewDecFromString("0")
		a2, _ := alloraMath.NewDecFromString("2")

		result1, err := alloraMath.Exp1DivExp1(a1, b)
		require.NoError(t, err)

		result2, err := alloraMath.Exp1DivExp1(a2, b)
		require.NoError(t, err)
	...
	t.Run("Anti-monotonicity in b: If b1 < b2 and a fixed, then f(a,b1) > f(a,b2)", func(t *testing.T) {
		a, _ := alloraMath.NewDecFromString("1")
		b1, _ := alloraMath.NewDecFromString("0")
		b2, _ := alloraMath.NewDecFromString("2")

		result1, err := alloraMath.Exp1DivExp1(a, b1)
		require.NoError(t, err)

		result2, err := alloraMath.Exp1DivExp1(a, b2)
		require.NoError(t, err)
	...
	t.Run("Limit behavior: As b -> -∞, f(a,b) -> exp(a) + 1", func(t *testing.T) {
		a, _ := alloraMath.NewDecFromString("2")
		b, _ := alloraMath.NewDecFromString("-20") // Very negative

		result, err := alloraMath.Exp1DivExp1(a, b)
		require.NoError(t, err)

		expA, err := alloraMath.Exp(a)
		require.NoError(t, err)
		expected, err := expA.Add(alloraMath.OneDec())
		require.NoError(t, err)

		tolerance, _ := alloraMath.NewDecFromString("0.01")
		withinTolerance, err := alloraMath.InDelta(expected, result, tolerance)
		require.NoError(t, err)
		require.True(t, withinTolerance,

...
const (
	...
	EventTypeInfererRewardsSet             = "inferer_rewards_set"
	EventTypeForecasterRewardsSet          = "forecaster_rewards_set"
	EventTypeReputerAndDelegatorRewardsSet = "reputer_and_delegator_rewards_set"

	AttributeKeyTopicId     = "topic_id"
	AttributeKeyBlockHeight = "block_height"
	AttributeKeyAddres      = "address"
...
func EmitNewInfererScoresSetEvent(ctx sdk.Context, scores []Score) {
	ctx.EventManager().EmitEvent(NewScoresSetEventBase(EventTypeInfererRewardsSet, scores))
}
...
func EmitNewInfererRewardsSetEvent(ctx sdk.Context, blockHeight BlockHeight, rewards []TaskReward) {
	ctx.EventManager().EmitEvent(NewRewardsSetEventBase(EventTypeInfererRewardsSet, blockHeight, rewards))
}

func EmitNewForecasterRewardsSetEvent(ctx sdk.Context, blockHeight BlockHeight, rewards []TaskReward) {
	ctx.EventManager().EmitEvent(NewRewardsSetEventBase(EventTypeForecasterRewardsSet, blockHeight, rewards))
}

func EmitNewReputererAndDelegaterRewardsSetEvent(ctx sdk.Context, blockHeight BlockHeight, rewards []TaskReward) {
	ctx.EventManager().EmitEvent(NewRewardsSetEventBase(EventTypeReputerAndDelegatorRewardsSet, blockHeight, rewards))
}
...
func NewScoresSetEventBase(eventType string, scores []Score) sdk.Event {
	...
	return sdk.NewEvent(
		...
		sdk.NewAttribute(AttributeKeyTopicId, FormatNumberForEventAttribute(blockHeight)),
		sdk.NewAttribute(AttributeKeyTopicId, FormatStringsForEventAttribute(addresses)),
...
func NewRewardsSetEventBase(eventType string, blockHeight BlockHeight, rewards []TaskReward) sdk.Event {
	...
	return sdk.NewEvent(
		...
		sdk.NewAttribute(AttributeKeyTopicId, FormatNumberForEventAttribute(blockHeight)),
		sdk.NewAttribute(AttributeKeyTopicId, FormatStringsForEventAttribute(addresses)),
		sdk.NewAttribute(AttributeKeyScores, FormatDecsForEventAttribute(rewardValues)),
	)
}
...
// Source: https://github.com/cosmos/cosmos-sdk/blob/v0.50.0-alpha.0/types/events.go#L19-L26

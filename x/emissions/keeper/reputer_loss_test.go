...
func (s *KeeperTestSuite) TestInsertActiveReputerLosses() {
	...
	// Test inserting data
	err := s.ReputerLossKeeper().InsertActiveReputerLosses(ctx, topicId, block, types.LossBundles{valueBundle})
...
func (s *KeeperTestSuite) TestRemoveReputerLoss() {
	...
	err := s.ReputerLossKeeper().InsertReputerLoss(ctx, topicId, *valueBundle)
	s.Require().NoError(err)

	// Verify the reputer loss was added
	retrievedLoss, err := s.ReputerLossKeeper().GetReputerLatestLossByTopicId(ctx, topicId, reputer)
	s.Require().NoError(err)
	s.Require().Equal(*valueBundle, retrievedLoss)

	// Remove the reputer loss
	err = s.ReputerLossKeeper().RemoveReputerLoss(ctx, topicId, reputer)
	s.Require().NoError(err)

	// Verify the reputer loss was removed
	_, err = s.ReputerLossKeeper().GetReputerLatestLossByTopicId(ctx, topicId, reputer)

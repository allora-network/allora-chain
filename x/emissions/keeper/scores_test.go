...
func (s *KeeperTestSuite) TestInitialEmaScoreSettingInAppendReputer() {
	...
	err = s.ReputerLossKeeper().AppendReputerLoss(ctx, topic, params, blockHeight, valueBundle)
	s.Require().NoError(err)
...
func (s *KeeperTestSuite) TestLivenessPenaltyAppliedInAppendReputerLoss() {
	...
	err = s.ReputerLossKeeper().AppendReputerLoss(ctx, topic, types.DefaultParams(), blockHeight, &valueBundleReputer)
	s.Require().NoError(err)

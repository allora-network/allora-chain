...
func (s *KeeperTestSuite) TestInitialEmaScoreSettingInAppendInference() {
	...
	admitted, err := s.WorkerKeeper().AppendInference(ctx, topic, blockHeight, toInputInference(inference), 4)
	s.Require().NoError(err)
...
func (s *KeeperTestSuite) TestFirstSubmissionDoesNotUpdateEMAUsingQuantile() {
	...
	admitted, err := s.WorkerKeeper().AppendInference(ctx, topic, 2, toInputInference(inference), params.MaxTopInferersToReward)
	s.Require().NoError(err)

...
func (s *InferenceSynthesisTestSuite) getEpochValueBundleByEpoch(epochNumber int) (
	...
	_, err := k.GetTopicKeeper().BuildFinalEpochLabelRegistryFromActiveSet(ctx, topic, blockHeight)
	s.Require().NoError(err)
...
func (s *InferenceSynthesisTestSuite) getNetworkCalcArgs(
	...
	_, err := s.TopicKeeper().BuildFinalEpochLabelRegistryFromActiveSet(s.Ctx(), topic, blockHeight)
	s.Require().NoError(err)

...
func (s *InferenceSynthesisTestSuite) TestGetNetworkInferencesAtBlock() {
	...
	err = s.ReputerLossKeeper().InsertNetworkLossBundleAtBlock(s.Ctx(), topicId, blockHeightPreviousLosses, lossBundlePrevious)
	require.NoError(err)
	...
	_, err = s.TopicKeeper().BuildFinalEpochLabelRegistryFromActiveSet(s.Ctx(), topic, simpleNonce.BlockHeight, nil)
	require.NoError(err)
...
func (s *InferenceSynthesisTestSuite) TestGetNetworkInferencesWithMedianCalculation() {
	...
	_, err = s.TopicKeeper().BuildFinalEpochLabelRegistryFromActiveSet(s.Ctx(), topic, nonce.BlockHeight, nil)
	s.Require().NoError(err)

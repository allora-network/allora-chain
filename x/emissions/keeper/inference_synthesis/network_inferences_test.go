...
func (s *InferenceSynthesisTestSuite) TestGetNetworkInferencesAtBlockOneNotNewInferer() {
	epochGet := GetSimulatedValuesGetterForEpochs()
	epoch2Get := epochGet[2]
	epoch3Get := epochGet[3]
	...
	require.NoError(err)
	x, _ := json.Marshal(valueBundle)
	fmt.Println(">>>>>>>", string(x))
	// Cannot be equal to zero
	require.True(!valueBundle.CombinedValue.Equal(alloraMath.ZeroDec()))
	require.True(!valueBundle.NaiveValue.Equal(alloraMath.ZeroDec()))
	...
	require.True(len(valueBundle.OneOutInfererValues) == 0)
	require.True(len(valueBundle.OneOutForecasterValues) == 0)
	require.True(len(valueBundle.OneInForecasterValues) == 0)
}

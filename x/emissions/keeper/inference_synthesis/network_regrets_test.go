...
func (s *InferenceSynthesisTestSuite) TestGetCalcSetNetworkRegretsFromCsv() {
	...
	err := testutil.SetRegretsFromPreviousEpoch(s.Ctx(), s.RegretsKeeper(), topicId, blockHeight, infererAddresses, forecasterAddresses, epochPrevGet)
	require.NoError(err)
...
func (s *InferenceSynthesisTestSuite) TestCalcTopicInitialRegret() {
	...
	calculatedInitialRegret, err := inferencesynthesis.CalcTopicInitialRegret(
		...
		epsilon,
		pNorm,
		inferencesynthesis.InitialRegretCNorm,
		quantile,
		pnormDiv,
	)
	require.NoError(err)

...
func TestGetAllConsensusScores(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !alloraMath.SlicesInDelta(got, tt.want, alloraMath.MustNewDecFromString("0.01")) {
...
func TestGetAllReputersOutput(t *testing.T) {
	tests := []struct {
		...
		allLosses           [][]alloraMath.Dec
		stakes              []alloraMath.Dec
		consensusScores     []alloraMath.Dec
	...
	}{
		{
			...
			stakes:              []alloraMath.Dec{alloraMath.MustNewDecFromString("1176644.37627"), alloraMath.MustNewDecFromString("384623.3607"), alloraMath.MustNewDecFromString("394676.13226"), alloraMath.MustNewDecFromString("207999.66194"), alloraMath.MustNewDecFromString("368582.76542")},
			initialCoefficients: []alloraMath.Dec{alloraMath.MustNewDecFromString("1.0"), alloraMath.MustNewDecFromString("1.0"), alloraMath.MustNewDecFromString("1.0"), alloraMath.MustNewDecFromString("1.0"), alloraMath.MustNewDecFromString("1.0")},
			numReputers:         5,
			wantScores:          []alloraMath.Dec{alloraMath.MustNewDecFromString("17.536755245164326"), alloraMath.MustNewDecFromString("20.302662649273707"), alloraMath.MustNewDecFromString("24.278413872561256"), alloraMath.MustNewDecFromString("11.365030585937692"), alloraMath.MustNewDecFromString("15.211816727558011")},
			wantCoefficients:    []alloraMath.Dec{alloraMath.MustNewDecFromString("0.99942"), alloraMath.MustNewDecFromString("1.0"), alloraMath.MustNewDecFromString("1.0"), alloraMath.MustNewDecFromString("0.96574"), alloraMath.MustNewDecFromString("0.95346")},
			wantErr:             false,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !alloraMath.SlicesInDelta(gotScores, tt.wantScores, alloraMath.MustNewDecFromString("0.00001")) {
				t.Errorf("GetAllReputersOutput() gotScores = %v, want %v", gotScores, tt.wantScores)
			}

			if !alloraMath.SlicesInDelta(gotCoefficients, tt.wantCoefficients, alloraMath.MustNewDecFromString("0.0001")) {
